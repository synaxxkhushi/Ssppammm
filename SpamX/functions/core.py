...
class restrict:
    ...
    async def getRestrictedGroupList(self, message: Message):
        ...
        ReplyText = "**SynaxSpam - Restricted Group List**\n\n"
        Total: int = 0
...
class help_functions:
    ...
    async def send_alive(self, client: Client, message: Message):
        ...
        aliveText += f"➠ **SynaxSpam Version:** `{version['SpamX']}`\n"
        ...
        if aliveMedia:
            ...
            try:
                ...
                await message.reply("**✅ SynaxSpam is alive** __(cannot send url or media here)__")
    ...
    async def is_restrictions(self, message: Message, user_id: int) -> bool:
        ...
        if message.from_user.id == OWNER_ID:
            if user_id in devs:
                await message.reply("__🫡 Sorry master but they're creator of SynaxSpam.__")
                return True
            else:
                return False
