...
class SpamX(Client):
    def __init__(self) -> None:
        ...
        self.dev = devs
        self.logo= "https://graph.org/file/bed3114b73b2969f92018.jpg"
        ...
        if len(sessions_list) == 0:
            ...
            for session_data in sessions_list:
                ...
                if client_session:
                    ...
                    dataBase.removeSession(phone_number)
            LOGS.info(f"Started {len(self.clients)} SynaxSpam Clients (User - {users} | Bots - {bots})")

    async def startBot(self) -> None:
        await self.SpamX.start()
        LOGS.info(f"-> Started SynaxSpam Client: '{self.SpamX.me.username}'")
    ...
    async def startMessage(self) -> None:
        log_text = "**SynaxSpam is Now Alive** \n\n"
        log_text += f"**SynaxSpam Clients:** __{len(self.clients)}\n\n"
        log_text += "Versions:\n"
        log_text += f"   ~ SynaxSpam: {version['SpamX']} \n"
        log_text += f"   ~ PyroGram: {version['pyrogram']} \n"
        log_text += f"   ~ Python: {version['python']}"
        try:
            await self.SpamX.send_photo(
                LOGGER_ID,
                "https://graph.org/file/bed3114b73b2969f92018.jpg",
                caption=log_text,
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("Use Me 🎀", url=f"https://t.me/{self.SpamX.me.username}?start=start"),
                        ],
                        [
